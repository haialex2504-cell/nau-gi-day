...
export default function ShareModal({ 
  ...
  // The full URL of the recipe page
  ...
  const shareToZalo = () => {
    // Note: Zalo intent requires a registered app-id usually, but this generic intent often works or opens the web interface.
  ...
  const shareToPinterest = () => {
    // Pinterest can take an explicit media query param
  ...
  const handleNativeShare = async () => {
    // Try Web Share API (Default on mobile devices)
    if (navigator.share) {
      try {
        await navigator.share({
          title: `Nấu Gì Đây: ${recipeName}`,
          text: `Cùng xem cách làm món ${recipeName} siêu ngon trên Nấu Gì Đây nhé!`,
          url: shareUrl,
        });
        onClose();
      } catch (error) {
      ...
      // Fallback to copy link if Web Share isn't available
      copyToClipboard();
    }
  };
        ...
        <div className="w-full max-w-md bg-surface-container-lowest/80 backdrop-blur-xl rounded-t-[2rem] p-6 shadow-[0_-8px_40px_rgba(28,28,22,0.12)]">
          ...
          <div className="bg-surface-container-low rounded-xl p-3 mb-8 flex items-center gap-4 border border-outline-variant/20 shadow-sm">
            ...
            <div className="flex-1 min-w-0">
              <p className="text-[10px] font-bold text-secondary uppercase tracking-widest mb-0.5 truncate">Gợi ý món hay</p>
          ...
          <div className="grid grid-cols-4 gap-y-8 mb-10">
            ...
            <div className="flex flex-col items-center gap-2 group cursor-pointer" onClick={() => {
              copyToClipboard();
              alert('Link đã được copy. Vui lòng dán vào app Instagram để chia sẻ!');
            }}>
            ...
            <div className="flex flex-col items-center gap-2 group cursor-pointer" onClick={copyToClipboard}>
              ...
              <span className="font-label text-xs font-semibold text-on-surface-variant">Copy Link</span>
            </div>
            ...
            <div className="flex flex-col items-center gap-2 group cursor-pointer" onClick={handleNativeShare}>
              ...
              <span className="font-label text-xs font-semibold text-on-surface-variant">Thêm</span>
            </div>
