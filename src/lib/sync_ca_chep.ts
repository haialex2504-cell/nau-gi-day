...
async function syncNewRecipes() {
  console.log("🚀 Starting sync process for new recipes...");

  const dataPath = path.join(process.cwd(), 'src/lib/recipes_data.json');
  const recipes = JSON.parse(fs.readFileSync(dataPath, 'utf8'));

  // Filter for new recipes only (ca-chep-)
  const newRecipes = recipes.filter((r: any) => r.id.startsWith('ca-chep-'));
  ...
  // Upserting ingredients (will not duplicate if name exists)
  ...
  if (ingError) {
    console.error("❌ Ingredient upsert failed:", ingError);
    throw ingError;
  }
  
  // Since we might have upserted some and kept others, we need ALL relevant ingredients to map correctly
  // Better to just fetch all ingredients once for the map
