...
async function syncNewRecipes() {
  console.log("🚀 Starting sync process for new ca-qua recipes...");

  const dataPath = path.join(process.cwd(), 'src/lib/recipes_data.json');
  const recipes = JSON.parse(fs.readFileSync(dataPath, 'utf8'));

  // Filter for new recipes only (ca-qua-)
  const newRecipes = recipes.filter((r: any) => r.id.startsWith('ca-qua-'));
